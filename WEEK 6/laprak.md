...
Buatlah implementasi ADT Doubly Linked list sebagai berikut di dalam file "Doublylist.h":
...
**Procedure/Function yang diimplementasikan:**
...
- procedure deleteByNopol(input/output L : List, input nopol : string)
- procedure printInfo(L : List)

Buatlah implementasi ADT pada file "Doublylist.cpp" dan coba hasil implementasi ADT pada file "main.cpp"
...
Program ini mengimplementasikan fungsi pencarian element (findElm) pada Doubly Linked List untuk mencari data kendaraan berdasarkan nomor polisi.
...
**Parameter:**
- `L` (List): List yang akan dicari
- `x` (infotype): Data kendaraan yang dicari (berisi nopol yang akan dicari)

**Return:**
- `address`: Pointer ke node yang ditemukan, atau Nil jika tidak ditemukan

**Algoritma:**
1. Inisialisasi pointer P = L.first (mulai dari node pertama)
2. Lakukan perulangan selama P != Nil:
   - Bandingkan P->info.nopol dengan x.nopol
   - Jika sama: return P (node ditemukan)
   - Jika tidak: P = P->next (lanjut ke node berikutnya)
3. Jika sampai akhir list tidak ditemukan: return Nil
...
2. Mengalokasi dan menambahkan 3 data kendaraan:
   ...
   - D003: putih, tahun 70
   - D004: kuning, tahun 90
3. Membuat variabel infotype untuk pencarian dengan nopol "D001"
4. Memanggil findElm untuk mencari kendaraan dengan nopol D001
5. Jika ditemukan:
   - Menampilkan "Masukkan Nomor Polisi yang dicari : D001"
   - Menampilkan detail kendaraan (nopol, warna, tahun)
...
**Karakteristik Fungsi findElm:**

1. **Linear Search**: Menggunakan metode sequential search dari awal hingga akhir list
2. **Kriteria Pencarian**: Berdasarkan nomor polisi (field nopol dari struct kendaraan)
3. **Early Return**: Fungsi berhenti segera setelah menemukan data yang cocok
4. **Traversal Forward**: Memanfaatkan pointer next untuk bergerak maju

**Kompleksitas:**
- **Best Case**: O(1) - jika elemen ditemukan di node pertama
- **Average Case**: O(n/2) - jika elemen ada di tengah list
- **Worst Case**: O(n) - jika elemen di akhir atau tidak ada dalam list
- **Space Complexity**: O(1) - hanya menggunakan variabel pointer tambahan

**Keunggulan:**
- Sederhana dan mudah diimplementasikan
- Tidak memerlukan list terurut
- Dapat dihentikan segera setelah menemukan data

**Penggunaan dalam Program:**

Fungsi findElm sangat berguna untuk:
...
Berdasarkan praktikum yang telah dilakukan tentang Doubly Linked List, dapat disimpulkan beberapa hal penting sebagai berikut:

1. **Doubly Linked List** merupakan struktur data linear yang memiliki dua pointer pada setiap node (next dan prev), memungkinkan traversal dua arah (forward dan backward). Struktur ini memberikan fleksibilitas lebih tinggi dibandingkan Single Linked List dalam operasi manipulasi data [1][2].

2. **Struktur Node Doubly Linked List** terdiri dari tiga komponen: info/data (menyimpan informasi), pointer next (menunjuk ke node berikutnya), dan pointer prev (menunjuk ke node sebelumnya). List memiliki dua pointer utama yaitu First (pointer ke node pertama) dan Last (pointer ke node terakhir) [1][3].

3. **Operasi Insert pada Doubly Linked List** mencakup insertFirst, insertLast, insertAfter, dan insertBefore. Setiap operasi insert memerlukan update pada dua pointer (next dan prev) untuk menjaga konsistensi hubungan antar node. Keberadaan pointer Last membuat operasi insertLast menjadi O(1) yang sangat efisien [3].

4. **Operasi Update** seperti updateFirst, updateLast, updateAfter, dan updateBefore dapat dilakukan dengan mudah pada Doubly Linked List. Operasi updateBefore merupakan keunggulan dari doubly linked list karena dapat mengakses node sebelumnya secara langsung tanpa perlu traversal dari awal [2].

5. **Operasi Search (findElm)** pada Doubly Linked List menggunakan algoritma linear search dengan kompleksitas O(n). Fungsi ini melakukan traversal dari First hingga Last untuk mencari node dengan kriteria tertentu (seperti nomor polisi pada kendaraan). Meskipun kompleksitasnya O(n), akses bidirectional memberikan fleksibilitas dalam implementasi [1].

6. **Operasi Delete** pada Doubly Linked List memiliki empat kasus utama: delete satu-satunya elemen, delete elemen pertama, delete elemen terakhir, dan delete elemen tengah. Keunggulan doubly linked list adalah dapat mengakses node sebelumnya (prev) secara langsung, membuat operasi delete lebih efisien dengan kompleksitas O(1) setelah node ditemukan [3].

7. **Implementasi ADT (Abstract Data Type)** Doubly Linked List memisahkan antara spesifikasi (interface) dan implementasi. File header (.h) berisi deklarasi struktur dan prototipe fungsi, sedangkan file implementasi (.cpp) berisi detail algoritma. Pendekatan ini meningkatkan modularitas, reusability, dan maintainability kode [2].

8. **Manajemen Memori** sangat penting dalam implementasi Doubly Linked List. Fungsi alokasi menggunakan operator `new` untuk membuat node baru, dan fungsi dealokasi menggunakan operator `delete` untuk membebaskan memori. Setiap operasi delete harus diikuti dengan dealokasi untuk mencegah memory leak [1].

9. **Kompleksitas Operasi** pada Doubly Linked List bervariasi: CreateList O(1), insertFirst/insertLast O(1), findElm O(n), deleteFirst/deleteLast O(1), dan printInfo O(n). Keberadaan pointer Last membuat operasi pada ujung list sangat efisien dibandingkan Single Linked List yang memerlukan traversal O(n) [3].

10. **Aplikasi Praktis** Doubly Linked List sangat berguna dalam implementasi browser navigation (back/forward), music player playlist, undo-redo functionality, LRU cache, dan sistem yang memerlukan navigasi bidirectional. Struktur ini optimal untuk aplikasi yang sering melakukan operasi insert/delete di kedua ujung list [2][3].
