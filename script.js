...
function checkUserSession() {
    const user = DB.get('current_user', null);
    if (user) {
        currentUser = user;
        updateUserUI(user);
    } else {
...
function switchAuthMode(e) {
    ...
    if (isRegister) {
        // Switch to Register
        title.textContent = 'Daftar Akun Bar';
        regFields.style.display = 'block';
        btn.textContent = 'Daftar';
        switchText.textContent = 'Sudah punya akun?';
        switchLink.textContent = 'Login sekarang';
...
function registerUser(name, email, password) {
    ...
    const newUser = { id: Date.now(), name, email, password }; // Password should be hashed in real app
    users.push(newUser);
    DB.set('users', users);
...
function loginUser(email, password) {
    ...
    if (user) {
        currentUser = user;
        DB.set('current_user', user); // Persist session
        updateUserUI(user);
        toggleAuthModal();
...
function approveSubmission(id) {
    ...
    renderMap && renderMap();
    renderAdminPanel && renderAdminPanel();
    showToast('âœ… Kontribusi berhasil disetujui!', 'success');
}
...
function rejectSubmission(id, reason) {
    ...
    localStorage.setItem('submissionHistory', JSON.stringify(history));
    renderAdminPanel && renderAdminPanel();
    showToast('âŒ Kontribusi ditolak: ' + reason, 'info');
}
...
function renderMap() {
    try {
        ...
        kulinerData.forEach((item, index) => {
            const iconEmoji = item.keliling ? 'ðŸ›µ' : 'ðŸ '; // Emoji based icons moved inside loop
                ...
                .bindPopup(`
                      <div class="popup-content">
                          ...
                          <span class="popup-badge ${item.keliling ? 'keliling' : 'tetap'}">
                              ${item.keliling ? 'ðŸ›µ Keliling' : 'ðŸ  Tetap'}
                          </span>
                          <p>${item.kategori}</p>
...
function showDetail(index) {
    try {
        ...
        const getHalalLabel = (halal) => {
            switch (halal) {
                case 'halal': return '<span class="halal-badge halal-mui">âœ… Halal MUI</span>';
                case 'halal-self': return '<span class="halal-badge halal-self">ðŸ•Œ Halal (Self-Declared)</span>';
                default: return '<span class="halal-badge halal-unknown">â“ Belum Diketahui</span>';
            }
        };
        ...
        const content = `
                ...
                <div class="detail-badges">
                    ...
                    <span class="tipe-badge ${item.keliling ? 'keliling' : 'tetap'}">${item.keliling ? 'ðŸšš Keliling' : 'ðŸ  Tetap'}</span>
                    ${ownerBadge}
                </div>
...
function submitNewKuliner() {
    // FR-10: Authenticated User Only (Contribution)
    if (!currentUser) {
        alert("Silakan login untuk menambahkan kuliner baru.");
        toggleAuthModal();
        return;
    }
    try {
...
function toggleFavorite(index) {
    // FR-09: Authenticated User Only
    if (!currentUser) {
        alert("Silakan login untuk menyimpan kuliner ke Favorit.");
        toggleAuthModal();
        return;
    }
...
function getWeatherBasedRecommendation(weatherData) {
    ...
    if (isDay) {
        if (condition.includes('rain')) {
            recommendation = "ðŸŒ§ï¸ Hujan di siang hari? Waktunya menikmati yang hangat! Ini rekomendasinya:";
            recommendedItems = kulinerData.filter(item => ["Soto", "Bakso", "Makanan Berat"].includes(item.kategori) && !item.keliling);
        } else if (temp > 29) {
            recommendation = "â˜€ï¸ Terik sekali! Segarkan diri dengan yang dingin dan menyegarkan:";
            recommendedItems = kulinerData.filter(item => item.kategori === "Minuman" || item.nama.toLowerCase().includes("es"));
        } else {
            recommendation = "ðŸŒž Cuaca cerah! Ini beberapa pilihan kuliner yang cocok dinikmati siang ini:";
            recommendedItems = kulinerData.filter(item => !item.keliling).slice(0, 10);
        }
    } else { // Night
        if (condition.includes('rain')) {
            recommendation = "ðŸŒƒ Hujan malam-malam, enaknya makan yang berkuah dan hangat. Coba ini:";
            recommendedItems = kulinerData.filter(item => ["Soto", "Bakso", "Sate"].includes(item.kategori) && !item.keliling);
        } else {
            recommendation = "ðŸŒ™ Malam yang indah untuk kulineran! Ini beberapa rekomendasi hangat untukmu:";
    ...
    if (recommendedItems.length > 0) {
        ...
        let itemText = selectedItems.map(item => `<br>ðŸ”¥ <b>${item.nama}</b> (${item.kategori})`).join('');
...
async function fetchWeather() {
    try {
        ...
        document.getElementById("weatherDetailsTemp").textContent = `${Math.round(data.main.temp)}Â°C`;
        document.getElementById("weatherMinTemp").textContent = `${Math.round(data.main.temp_min)}Â°C`;
        document.getElementById("weatherMaxTemp").textContent = `${Math.round(data.main.temp_max)}Â°C`;
...
function simulateWeather() {
    ...
    document.getElementById("weatherTemp").textContent = `${weatherData.main.temp}Â°C`;
    document.getElementById("weatherDetailsTemp").textContent = `${weatherData.main.temp}Â°C`;
    document.getElementById("weatherMinTemp").textContent = `${weatherData.main.temp_min}Â°C`;
    document.getElementById("weatherMaxTemp").textContent = `${weatherData.main.temp_max}Â°C`;
...
async function installPWA() {
    ...
    if (outcome === 'accepted') {
        showToast('Aplikasi berhasil diinstall! ðŸŽ‰', 'success');
    }
...
window.addEventListener('online', () => {
    ...
    showToast('Kamu kembali online! ðŸŒ', 'success');
});

window.addEventListener('offline', () => {
    showOfflineIndicator();
    showToast('Kamu sedang offline ðŸ“¡', 'warning');
});
        ...
        (position) => {
            ...
            if (map) {
                ...
                L.marker([latitude, longitude], {
                    icon: L.divIcon({
                        html: '<div class="user-marker">ðŸ“</div>',
                        className: '',
                        iconSize: [40, 40]
                    })
                }).addTo(map).bindPopup('Lokasi Anda').openPopup();
            }
            showToast('Lokasi ditemukan! ðŸ“', 'success');
        },
        () => {
...
function startVoiceSearch() {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        ...
        recognition.start();
        showToast('Mendengarkan... ðŸŽ¤', 'info');
    } else {
...
function scrollToMap() {
    ...
    showToast('Peta Kuliner ðŸ—ºï¸', 'info');
}
...
function showExplore() {
    ...
    list.innerHTML = `
            <div class="explore-section">
                <h3 style="padding: 16px; color: var(--primary);">ðŸ§­ Jelajahi Kategori</h3>
        ...
        `;
    showToast('Jelajahi Kategori ðŸ§­', 'info');
}
...
function showFavorites() {
    ...
    if (favData.length === 0) {
        list.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Simpan kuliner favoritmu dengan menekan tombol â¤ï¸ di detail kuliner</p>
    ...
    list.innerHTML = '<h3 style="padding: 16px; color: var(--primary);">â¤ï¸ Kuliner Favorit Kamu</h3>';
    ...
    list.appendChild(cardContainer);
    showToast(`${favData.length} Favorit â¤ï¸`, 'success');
}
...
function showNews() {
            ...
            <div class="lm-news-grid">
                ${news.map(n => `
                    <article class="lm-news-card" role="article">
                        ...
                        <h3>${n.title}</h3>
                        <div class="news-meta">${n.category} â€¢ ${new Date(n.publishedAt).toLocaleDateString('id-ID')}</div>
                        <p>${(n.summary || n.content || '').slice(0, 180)}${(n.content || '').length > 180 ? '...' : ''}</p>
                        <p><a href="#" onclick="showNewsDetail(${n.newsId});return false;">Baca selengkapnya â†’</a></p>
                    </article>
                `).join('')}
            </div>
        `;
    ...
    list.innerHTML = html;
    showToast('Berita & Promo ðŸ“°', 'info');
}
...
function showNewsDetail(newsId) {
    ...
    list.innerHTML = `
            <div class="lm-news-detail">
                <button class="lm-news-back" onclick="showNews()">â† Kembali ke Berita</button>
                <h1>${n.title}</h1>
                <div class="news-meta">${n.category} â€¢ ${new Date(n.publishedAt).toLocaleDateString('id-ID')}</div>
...
function toggleAuthModal() {
    showToast('Fitur Login segera hadir! ðŸ”', 'info');
}
...
// Expose Data Kuliner untuk Chatbot AI
window.getAllKulinerData = function () {
    // Coba ambil dari variable global kulinerData, jika gagal ambil dari initial
    if (typeof kulinerData !== 'undefined' && Array.isArray(kulinerData)) {
        return kulinerData;
    }
    return typeof initialKulinerData !== 'undefined' ? initialKulinerData : [];
};
